...
Runnable Pest sample with covers() ownership checks and Parity in CI.
...
```bash
cd code && composer install && XDEBUG_MODE=coverage ./vendor/bin/pest --coverage-clover=clover.xml --colors=never && cd ..
